...
void meow::gpu::uniform::registry(std::string_view key) {
  this->program_location_map[key] = glGetUniformLocation(this->linked_program, key.data());
}

uint32_t &meow::gpu::uniform::operator[](std::string_view key) {
  return this->program_location_map[key];
}

bicudo::result_t meow::gpu_compile_shader_program(
  uint32_t *p_program,
  const std::vector<meow::gpu::shader> &shader_list
) {
  if (shader_list.empty()) {
    bicudo::loge("Invalid shader, empty resources");
  ...
  std::string msg {};
  for (const meow::gpu::shader &module : shader_list) {
    ...
    if (status == GL_FALSE) {
      ...
      bicudo::loge("Failed to compile shader '", p_src, "' stage: \n", msg);
      break;
    }
  ...
  if (keep) {
    ...
    if (status == GL_FALSE) {
      ...
      bicudo::loge("Failed to link program: \n", msg);
    }
  }
...
bicudo::result_t meow::gpu_dispatch_draw_call(
